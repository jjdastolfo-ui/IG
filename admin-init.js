...
module.exports = function(app, db, calcularIGUVentana) {
  ...
  // ==========================================================================
  // ENDPOINT NUEVO: Limpiar duplicados de precios_raw
  //
  // Borra registros duplicados dejando solo UNO por combinacion
  // (fecha, categoria_codigo, fuente), el mas reciente por created_at.
  //
  // Uso:
  //   curl "https://www.igu.com.uy/admin/limpiar-duplicados?secret=IGU_INIT_2026"
  // ==========================================================================
  app.get('/admin/limpiar-duplicados', (req, res) => {
    if (req.query.secret !== SECRET) return res.status(403).json({ error: 'No autorizado' });

    try {
      const antes = db.prepare(`SELECT COUNT(*) AS n FROM precios_raw`).get().n;

      // Estrategia: para cada combinacion (fecha, categoria, fuente),
      // mantener solo el registro con el id mas alto (el ultimo insertado)
      const duplicadosDetectados = db.prepare(`
        SELECT fecha, categoria_codigo, fuente, COUNT(*) AS cantidad
        FROM precios_raw
      ...
      `);
      const resultado = borrar.run();

      const despues = db.prepare(`SELECT COUNT(*) AS n FROM precios_raw`).get().n;
      ...
      res.json({
        ...
        borrados: resultado.changes,
        combinaciones_con_duplicados: duplicadosDetectados.length,
        detalle_duplicados: duplicadosDetectados.slice(0, 20)
      });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  console.log('Admin endpoints v2.2: /init-db, /actualizar-ponderaciones, /actualizar-bases-v22, /migrar-v22, /ejecutar-sql, /limpiar-duplicados, /importar-historico, /debug-excel, /recalcular-todo, /status');
};
