...
cron.schedule('15 * * * *', async () => {
  try {
    ...
    // El healthcheck ahora decide solo si amerita email (segun modo + severidad)
    if (resultado.debe_enviar_email) {
      try {
        ...
        if (envio.enviado) {
          ...
          console.log(`[healthcheck] Email no enviado: ${envio.motivo || 'razon desconocida'}`);
        }
      } catch (emailErr) {
...
cron.schedule('15 9 * * 1', async () => {
  ...
  try {
    ...
    // Enviar reporte semanal de auditoria (siempre)
    ...
    // ========================================================================
    // CAPA 3: Analisis con Claude API si hay anomalias
    // ========================================================================
    if (process.env.ANTHROPIC_API_KEY) {
      try {
        ...
        const resultadoAnalisis = await ejecutarAnalisisSiHayAnomalias(db, resultado, healthcheck);

        console.log(`[claude-analisis] ${resultadoAnalisis.accion}: ${resultadoAnalisis.mensaje || resultadoAnalisis.triggers_detectados + ' triggers'}`);
      } catch (claudeErr) {
        console.error('[claude-analisis] Error:', claudeErr.message);
      }
    } else {
      console.log('[claude-analisis] Saltado (ANTHROPIC_API_KEY no configurada)');
    }
  } catch (err) {
...
app.listen(PORT, () => {
  console.log('='.repeat(60));
  console.log('IGU v2.6 - Indice Ganadero Uruguayo');
  ...
  console.log(`  - Lunes 09:15 UY: auditoria veracidad + analisis Claude si hay anomalias`);
  console.log(`  - Cada hora :15:  healthcheck contextual (modo segun dia/hora)`);
  console.log(`Healthcheck modos:`);
  console.log(`  - Estructural (24/7): ponderaciones, categorias, integridad DB`);
  console.log(`  - Publicacion (lun 10-14h UY): ciclo del lunes completo`);
  const emailConfigurado = process.env.RESEND_API_KEY && process.env.EMAIL_TO;
  const claudeConfigurado = !!process.env.ANTHROPIC_API_KEY;
  console.log(`  Email alertas: ${emailConfigurado ? 'CONFIGURADO (' + process.env.EMAIL_TO + ')' : 'NO configurado'}`);
  console.log(`  Claude API:    ${claudeConfigurado ? 'CONFIGURADO' : 'NO configurado'}`);
  console.log('='.repeat(60));
});
